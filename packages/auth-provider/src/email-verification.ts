...
export const DEFAULT_EMAIL_VERIFICATION_LINK_TTL_MS = DEFAULT_LINK_TTL_MS;
...
export interface CreateEmailVerificationTokenOptions {
  ttlMs?: number;
}

export interface CreateEmailVerificationTokenResult {
  token: string;
  expiresAt: number;
  jti: string;
}

export interface VerifyEmailVerificationTokenResult extends EmailVerificationPayload {
  jti: string;
}

export interface SingleUseEmailVerificationStore {
...
export function createEmailVerificationToken(
  ...
  const ttlMs = options.ttlMs ?? DEFAULT_EMAIL_VERIFICATION_LINK_TTL_MS;
...
export function verifyEmailVerificationToken(
...
): VerifyEmailVerificationTokenResult | null {
  ...
  const store = options.usedTokenStore ?? defaultEmailVerificationStore;
...
export function createMemoryEmailVerificationStore(): SingleUseEmailVerificationStore {
...
const defaultEmailVerificationStore = createMemoryEmailVerificationStore();

export function createNoOpEmailVerificationStore(): SingleUseEmailVerificationStore {
  return {
    isUsed: () => false,
    markUsed: () => {},
  };
